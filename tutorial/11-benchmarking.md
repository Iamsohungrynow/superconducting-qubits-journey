...
5. **Fit:** the oscillation frequency $\to \Delta f$ (correct $\omega_q$ by $2\pi\Delta f$ in angular-frequency units); the envelope $\to T_2^*$.
...
6. Leakage is outside this model; it must be measured separately, for example with leakage/seepage RB.
...
Standard RB reports coherent errors only through their average infidelity; it does not by itself tell you whether the error was coherent, stochastic, or dangerous in worst case. The SPAM-robust tool that separates them is **unitarity (purity) RB**: instead of survival probability, it tracks the *purity* of the output state vs sequence length. Depolarization drains purity; a pure over-rotation does not, so a high $r$ with high unitarity flags a *coherent* error you can calibrate away.
