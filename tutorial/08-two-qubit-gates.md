...
The simplest way to couple two transmons is to wire them together with a capacitor $C_g$. The shared capacitance produces an electrostatic energy proportional to the *product* of the two charges, $H_c \propto C_g\,V_1 V_2 \propto \hat n_1 \hat n_2$, a charge-charge interaction. Let us turn that into the familiar exchange term, one step at a time. In this chapter $g,\Delta,\alpha,\zeta$ are angular frequencies; Hamiltonians are written as $H/\hbar$ unless stated otherwise.
...
$$\boxed{\frac{H_\text{int}}{\hbar} = g\left(\sigma_+^{(1)}\sigma_-^{(2)} + \sigma_-^{(1)}\sigma_+^{(2)}\right)}, \qquad g \approx \tfrac{1}{2}\,\frac{C_g}{\sqrt{C_1 C_2}}\sqrt{\omega_1 \omega_2}.$$
...
Away from the $|01\rangle$-$|10\rangle$, $|11\rangle$-$|02\rangle$, and $|11\rangle$-$|20\rangle$ avoided crossings, where all relevant detunings are large compared with $g$, the perturbative residual shift is

$$\zeta_{ZZ} = \frac{E_{11}-E_{10}-E_{01}+E_{00}}{\hbar} \approx \frac{2g^2(\alpha_1+\alpha_2)}{(\Delta-\alpha_2)(\Delta+\alpha_1)}.$$
...
3. **Energy denominators.** With $\Delta=\omega_1-\omega_2$, the angular-frequency denominators are $(E_{11}-E_{02})/\hbar=\Delta-\alpha_2$ and $(E_{11}-E_{20})/\hbar=-(\Delta+\alpha_1)$. Thus
   $$\delta \omega_{11}= \frac{2g^2}{\Delta-\alpha_2}-\frac{2g^2}{\Delta+\alpha_1}.$$
...
after compensating the one-qubit phases. That conditional sign flip is the controlled-Z action, $\text{CZ}=\mathrm{diag}(1,1,1,-1)$.
...
Faster gates need a larger integrated conditional shift, obtained by larger $g$, a closer approach to the crossing, or a longer dwell. For a fixed linear sweep, larger $g$ increases the gap and suppresses Landau-Zener transitions; leakage grows when the trajectory is made faster or closer without adequate shaping. The fix is not raw speed but a **shaped trajectory** (the "fast-adiabatic"/Martinis-style pulse, DRAG-like derivative shaping). Two ways to run CZ in practice: (a) adiabatic flux tuning into the crossing, and (b) diabatic/resonant and modern tunable-coupler "net-zero" approaches.
...
A calibrated echoed-CR pulse usually realizes $ZX_{\pi/2}=\exp[-i(\pi/4)Z\otimes X]$ up to echoed terms and frame phases; single-qubit rotations then compile it to CNOT. Likewise, $\mathrm{CNOT}=(I\otimes H)\,\mathrm{CZ}\,(I\otimes H)$.
...
Frame the two-qubit error as a sum of channels, $\varepsilon_\text{2Q}\approx \varepsilon_\text{coh}+\varepsilon_\text{leak}+\varepsilon_\text{dec}$. For a heuristic error-probability estimate, the decoherence floor scales as
